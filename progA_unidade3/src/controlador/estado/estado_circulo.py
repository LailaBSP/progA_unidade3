...
class EstadoCirculo(Estado):
    """Representa o estado de construção de uma figura do tipo Círculo.

    Gerencia o ciclo de vida da criação de um círculo na tela através dos 
    ...
    @see modelo.figuras.Circulo
    @since 1.0
    """

    def __init__(self, controlador):
        """Inicializa o estado do círculo com uma referência ao controlador principal.
    ...
    def mouse_pressionado(self, event):
        """Cria uma nova instância de Círculo com base nas coordenadas do clique inicial (centro).

        @author Laila Beatriz
        @param event Evento do Tkinter contendo a posição x e y inicial do mouse.
        """
        self.controlador.modelo.figura_nova = Circulo(event.x, event.y)
        self.controlador.visao.atualizar_tela()

    def mouse_arrastado(self, event):
        """Atualiza o raio do círculo conforme o mouse é arrastado para longe do centro.
    ...
    def mouse_solto(self, event):
        """Finaliza o círculo atual, adicionando-o ao modelo caso esteja válido.
    ...
    def duplo_clique(self, event):
        """Método não utilizado para a ferramenta de Círculo.
